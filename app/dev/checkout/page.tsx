    ...
    return (
        <div className="p-10 max-w-5xl mx-auto space-y-6 bg-gray-950 text-gray-100 min-h-screen">
            <div>
                ...
                <p className="text-sm text-gray-400">Prueba las validaciones de Zod, Clerk y Prisma.</p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <form onSubmit={handleTestCheckout} className="space-y-4 border border-gray-800 p-5 rounded-lg bg-gray-900 shadow-xl">
                    ...
                    <button
                        type="submit"
                        disabled={loading}
                        className={`w-full py-2 rounded text-white font-medium transition-colors ${loading ? "bg-gray-700" : "bg-blue-600 hover:bg-blue-700"}`}
                    >
                ...
                <div className="flex flex-col space-y-4">
                    <h2 className="text-lg font-semibold border-b border-gray-800 pb-2 text-white">Respuesta</h2>
                    {status && (
                        <div className={`p-2 rounded text-sm font-bold w-fit ${status >= 200 && status < 300 ? "bg-green-950/80 border-green-800 text-green-400" : "bg-red-950/80 border-red-800 text-red-400"}`}>
                            Status: {status}
                        </div>
                    )}
                    <div className="flex-1 bg-black border border-gray-800 text-green-400 p-4 rounded-lg overflow-auto font-mono text-sm min-h-[340px]">
                        {response ? <pre>{JSON.stringify(response, null, 2)}</pre> : <span className="text-gray-600 italic">Haz clic en enviar...</span>}
                    </div>
                </div>
            </div>
        </div>
