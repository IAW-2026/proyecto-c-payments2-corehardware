...
export default function SideNav({ tabs }: { tabs: Tab[] }) {
    ...
    return (
        <nav className="flex md:flex-col justify-around md:justify-start md:w-52 md:h-screen md:sticky md:top-0 md:shrink-0 border-b md:border-b-0 md:border-r
            ...
            {tabs.map(({ label, href, icon }) => {
                ...
                return <Tab key={href} href={href} icon={icon}>{label}</Tab>
            })}
        </nav>
    )
}